...
download_generator() {
  if [[ ! -f "$GENERATOR" ]]; then
    mkdir -p "$ROOT/.cache"
    curl -o "$GENERATOR" "https://repo1.maven.org/maven2/org/openapitools/openapi-generator-cli/4.3.1/openapi-generator-cli-4.3.1.jar"
  fi
}
...
run_format() {
  ...
  for command in 'ruff check --fix -e .' 'pre-commit run --files .'; do
...
download_generator
mkdir -p src/tiledb/cloud/_common/api_v4
generate_api "${ABSPATH%/}/openapi.yaml" _common.api_v4
