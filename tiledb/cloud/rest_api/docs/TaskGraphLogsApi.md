...
from tiledb.cloud.rest_api.api import task_graph_logs_api
...
with tiledb.cloud.rest_api.ApiClient(configuration) as api_client:
    ...
    api_instance = task_graph_logs_api.TaskGraphLogsApi(api_client)
    namespace = "namespace_example" # str | The namespace that will own this task graph log.
    log = TaskGraphLog(
        namespace="namespace_example",
        name="name_example",
        status=TaskGraphLogStatus("submitted"),
        total_cost=3.14,
        access_cost=3.14,
        egress_cost=3.14,
        ...
        nodes=[
            TaskGraphNodeMetadata(
                ...
                run_location=TaskGraphLogRunLocation("server"),
                status=ArrayTaskStatus("QUEUED"),
            ),
        ],
    ) # TaskGraphLog | 
...
> TaskGraphLogsData list_task_graph_logs()
...
with tiledb.cloud.rest_api.ApiClient(configuration) as api_client:
    ...
    start_time = dateutil_parser('1970-01-01T00:00:00.00Z') # datetime | Include logs created after this time. (optional)
    end_time = dateutil_parser('1970-01-01T00:00:00.00Z') # datetime | Include logs created before this time. (optional)
...
import tiledb.cloud.rest_api
from tiledb.cloud.rest_api.api import task_graph_logs_api
from tiledb.cloud.rest_api.model.task_graph_client_node_status import TaskGraphClientNodeStatus
from tiledb.cloud.rest_api.model.error import Error
from pprint import pprint
# Defining the host is optional and defaults to http://localhost/v1
# See configuration.py for a list of all supported configuration parameters.
...
with tiledb.cloud.rest_api.ApiClient(configuration) as api_client:
    ...
    report = TaskGraphClientNodeStatus(
        client_node_uuid="client_node_uuid_example",
        status=ArrayTaskStatus("QUEUED"),
    ) # TaskGraphClientNodeStatus | The node status to report.
...
with tiledb.cloud.rest_api.ApiClient(configuration) as api_client:
    ...
    ) # TaskGraphLog | Updates to make to the task graph log. The only manual update that a client should need to make to a task graph log is to update its completion status to `succeeded`, `failed`, or `cancelled`. 
