...
def _ingest_tiff_udf(
    io_uris: Sequence[Tuple], key: str, secret: str, *, workers: int,
):
    """Internal udf that ingests server side tiff/svs files into tiledb arrays using tiledb-bioimg API
...
def _get_uris(source: Union[Sequence[str], str], output_dir: str):
    ...
    def create_output_path(input_file, output_dir):
        return os.path.join(output_dir, os.path.basename(parsed_uri) + ".tdb")

    if isinstance(source, str):
        # Single file input
        parsed_uri = source.decode().strip()
        return (parsed_uri, create_output_path(parsed_uri, output_dir))
    else:
        # List of input uris
        result = []
        output_uris = []
        for uri in source:
            parsed_uri = uri.decode().strip()
            result.append(parsed_uri)
            output_uris.append(create_output_path(parsed_uri, output_dir))
