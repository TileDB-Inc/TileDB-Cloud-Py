...
class Node(Generic[_T]):
    ...
    def finished(self) -> bool:
        ...
        if ft:
            return ft.done()
        return self.status is st.Status.CANCELLED
    ...
    def exec(self, namespace: Optional[str] = None):
        ...
        self._future = self.dag.executor.submit(self._do_exec)
        assert self._future  # Needed to silence mypy.
    ...
    def wait(self, timeout: Optional[float] = None):
        ...
        if ft:
            try:
                ...
                # It's still done, even if it had an exception.
                pass
            return
        while not self.done():
            time.sleep(0.1)
