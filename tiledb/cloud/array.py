...
def share_array(uri, namespace, permissions):
  ...
  try:
    return api_instance.share_array(namespace=array_namespace, array=array_name,
                                    array_sharing=rest_api.models.ArraySharing(namespace=namespace,
                                                                               actions=permissions))
  except GenApiException as exc:
    raise tiledb_cloud_error.check_exc(exc) from None
...
def register_array(uri, namespace=None, array_name=None, description=None, access_credentials_name=None):
  """
  ...
  try:
    return api_instance.register_array(namespace=namespace, array=uri,
                                       array_metadata=rest_api.models.ArrayMetadataUpdate(description=description,
                                                                                          name=array_name, uri=uri,
